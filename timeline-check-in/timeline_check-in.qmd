...
salinity_prcp <- salinity_filtered |> 
  # joining `monthly_prcp` to `salinity_filtered` by the columns month and year
...
precipitation_peaks <- monthly_prcp |>
  ...
  filter(date %in% c("2023-03-01", "2021-01-01", "2023-02-01", "2021-10-01")) |> 
...
ggplot(data = monthly_prcp,
  ...
  labs(x = "Date",
       ...
       # adding title
       title = "Figure 1. Total Monthly Precipitation at NCOS") + 
...
ggplot(data = salinity_prcp,
  ...
  labs(x = "Date",
       ...
       # adding title
       title = "Figure 2. Monthly Average NCOS Salinity",
       # adding legend title
...
ggplot(data = elv_viz,
  ...
  labs(x = "Monthly Average Water Surface Elevation (m)",
       y = "Monthly Average Salinity (ppt)",
       # adding title
       title = "Figure 3. How Salinity Relates to Water Surface Elevation (Venoco Bridge)") 
...
ggplot(data = exploratory_salinity,
  ...
  labs(x = "Elevation Code",
       y = "Mean Salinity (ppt)",
       # adding in title
       title = "Figure 4. Mean Salinity Differences Between Elevation Codes") +
...
ggplot(data = salinity_prcp,
  ...
  labs(x = "Monthly Precipitation (in)",
       y = "Monthly Average Salinity (ppt)",
       # adding figure title
       title = "Figure 5. How Precipitation Explains Salinity")
...
# First variable: mean salinity column from the elevation visualization dataframe
cor.test(elv_viz$mean_salinity,
         # second variable: average water surface elevation from the elevation visualization dataframe
         elv_viz$avg_elv,
...
ggplot(data = water_quality_weather |> 
         # removing outlier IDs, "other" sites, and keeping only elevation codes 0, 1, and 2
         filter(!global_id %in% salinity_outlier_ids,
                site_name != "other",
