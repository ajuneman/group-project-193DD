...
monthly_temp <- water_quality_weather |> 
  filter(elevation_code %in% c("0", "1", "2")) |> 
  filter(!site_name == "other") |> 
  distinct(month, year, wy, .keep_all = TRUE) |> 
  group_by(month, year) |> 
  ...
  ungroup() |> 
  mutate(avg_temp_f = (avg_temp*9/5) + 32) |> 
  mutate(date = lubridate::make_date(.data$year, .data$month, 1)) |> 
  mutate(month = factor(month,
                        levels = 1:12,
                        labels = c("Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"),
...
temp_viz <- monthly_temp |> 
  left_join(exploratory_salinity,  by = c("month", "year"))
...
elv_viz <- monthly_surfcae_elv |> 
  ...
  mutate(log_prcp = log(sum_prcp +1)) |> 
  filter(!avg_elv == "NaN")
